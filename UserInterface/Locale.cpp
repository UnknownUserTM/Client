...
void LocaleService_LoadConfig(const char* fileName)
{
	...
	{
		MULTI_LOCALE_CODE = CP_1250;
		strcpy(MULTI_LOCALE_NAME, MAIN_LOCALE_LANGUAGE);
		sprintf(MULTI_LOCALE_PATH, "locale/%s", MULTI_LOCALE_NAME);

		FILE * fpCodePage = fopen(FILE_NAME_MAIN_LOCALE, "w");
		if (!fpCodePage)
			return;

		fprintf(fpCodePage, "10002 %d %s", MULTI_LOCALE_CODE, MULTI_LOCALE_NAME);
		fclose(fpCodePage);
	}
	NANOEND
}
