...
bool CInstanceBase::IsAttackableInstance(CInstanceBase& rkInstVictim)
{
	...
	else if (IsPC())
	{
		...
		if (rkInstVictim.IsPC())
		{
			//std::string & c_rstrMapFileName = CPythonBackground::Instance().GetWarpMapName();
			if (CPythonBackground::Instance().IsInBattlefield())
				return true;
			
//			if (CPythonBackground::Instance().IsInBattlezone())
//				return true;	
