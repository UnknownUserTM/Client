...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_ITEM_PICKUP:
				ret = RecvPickupItemPacket();
				break;
				
			case HEADER_GC_SHOUT_FILTER_LIST:
				ret = RecvLoadShoutFilterListPacket();
				break;		
			...
			// case HEADER_GC_QUEST_INTRO_DESC:
				// ret = QuestIntroDesc();
				// break;
				
			// case HEADER_GC_QUEST_INTRO_REWARD:
				// ret = QuestIntroReward();
				// break;
				
			// case HEADER_GC_QUEST_INTRO_OBJECTIVE:
				// ret = QuestIntroObjective();
				// break;
...
// bool CPythonNetworkStream::QuestIntroDesc()
// {
	// TPacketGCQuestIntroDesc QuestIntroDesc;

	// if (!Recv(sizeof(TPacketGCQuestIntroDesc), &QuestIntroDesc))
	// {
		// Tracen("Recv TPacketGCQuestIntroDesc Packet Error");
		// return false;
	// }
	
	// PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_QuestIntro_Description", Py_BuildValue("(ss)", QuestIntroDesc.title, QuestIntroDesc.desc));
// }

// bool CPythonNetworkStream::QuestIntroReward()
// {
	// TPacketGCQuestIntroReward QuestIntroReward;

	// if (!Recv(sizeof(TPacketGCQuestIntroReward), &QuestIntroReward))
	// {
		// Tracen("Recv TPacketGCQuestIntroReward Packet Error");
		// return false;
	// }

	// PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_QuestIntro_Reward", Py_BuildValue("(sii)", QuestIntroReward.rewardType,QuestIntroReward.vnum,QuestIntroReward.count));
// }

// bool CPythonNetworkStream::QuestIntroObjective()
// {
	// TPacketGCQuestIntroObjective QuestIntroObjective;

	// if (!Recv(sizeof(TPacketGCQuestIntroObjective), &QuestIntroObjective))
	// {
		// Tracen("Recv TPacketGCQuestIntroObjective Packet Error");
		// return false;
	// }
	
	// PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_QuestIntro_Objective", Py_BuildValue("(si)", QuestIntroObjective.desc,QuestIntroObjective.status));
// }
...
bool CPythonNetworkStream::SendMessengerRequestWarpByNamePacket(const char * c_szName)
{
	...
	if (!Send(sizeof(szName), &szName))
		return false;
	Tracef(" SendMessengerRequestWarpByNamePacket : %s\n", c_szName);
	return SendSequence();
}
...
bool CPythonNetworkStream::SendMessengerSummonByNamePacket(const char * c_szName)
{
	...
	if (!Send(sizeof(szName), &szName))
		return false;
	Tracef(" SendMessengerSummonByNamePacket : %s\n", c_szName);
	return SendSequence();
}
...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	else
	{
		...
		else if (CHAT_TYPE_DEBUG == kChat.type)
		{
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_DebugMessage", Py_BuildValue("(s)", buf));
		}
		...
		CPythonChat::Instance().AppendChat(kChat.type, buf, kChat.bEmpire, kChat.szLanguage);

	}
	return true;
}

bool CPythonNetworkStream::SendShoutLanguageFilterPacket(int bSubHeader, int iType, const char * szText, BYTE * bFilterList)
{
	TPacketCGShoutFilter pack;
	pack.header = HEADER_CG_SHOUT_FILTER;
	pack.bSubHeader = bSubHeader;
	pack.iType = iType;
	strncpy(pack.szText, szText, sizeof(pack.szText));
	memcpy(&pack.bFilterList, bFilterList, sizeof(pack.bFilterList));
...
bool CPythonNetworkStream::RecvLoadShoutFilterListPacket()
{
	TPacketGCLoadShoutFilterList pack;
	if (!Recv(sizeof(pack), &pack))
		return false;
	
	for (int i = 0; i < _countof(pack.bFilterList); i++)
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_LoadShoutFilterList", Py_BuildValue("(ii)", i, pack.bFilterList[i]));
...
bool CPythonNetworkStream::RecvPickupItemPacket()
{
	...
	CPythonLanguageManager::Instance().RecvPickupItemPacket(pack.szBuf, pack.vnum);
	return true;
}
...
#else
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnGameOver", Py_BuildValue("ii", DeadPacket.t_d[0], DeadPacket.t_d[1]));	
#endif
			}
...
bool CPythonNetworkStream::RecvScriptPacket()
{
	...
	// PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SendQuestString", Py_BuildValue("(s)", str));
...
bool CPythonNetworkStream::RecvNPCList()
{
	...
	for (int i = 0; i < kNPCPosition.count; ++i)
	{
		...
		CPythonMiniMap::Instance().RegisterAtlasMark(NPCPosition.bType, ((CPythonNonPlayer::Instance().GetName(NPCPosition.dwVnum, &c_szConvertedName)) ? c_szConvertedName : NPCPosition.name), NPCPosition.x, NPCPosition.y);
	}
