...
void CPythonChat::SChatLine::Delete(CPythonChat::SChatLine* pkChatLine)
{
	...
	pkChatLine->InstanceLanguageFlag.Destroy();
	pkChatLine->InstanceEmpireFlag.Destroy();
...
{
	Instance.Destroy();
	InstanceLanguageFlag.Destroy();
	InstanceEmpireFlag.Destroy();
}
...
void CPythonChat::UpdateViewMode(DWORD dwID)
{
	...
	for (itor = pLineList->begin(); itor != pLineList->end();)
	{
		...
		int iWidth = pChatSet->m_ix, iLanguageFlagWidth = pChatLine->InstanceLanguageFlag.GetWidth(), iEmpireFlagWidth = pChatLine->InstanceEmpireFlag.GetWidth();

		if (iLanguageFlagWidth && iEmpireFlagWidth)
		{
			pChatLine->InstanceLanguageFlag.SetPosition(iWidth, pChatSet->m_iy + iHeight + 2);
			iWidth += iLanguageFlagWidth + 2;
				
			pChatLine->InstanceEmpireFlag.SetPosition(iWidth + 4, pChatSet->m_iy + iHeight + 2);
			iWidth += iEmpireFlagWidth + 8;
		}
...
void CPythonChat::Render(DWORD dwID)
{
	...
	for (TChatLineList::iterator itor = pLineList->begin(); itor != pLineList->end(); ++itor)
	{
		...
		CGraphicImageInstance & lInstance = (*itor)->InstanceLanguageFlag;
		lInstance.Render();
		
		CGraphicImageInstance & eInstance = (*itor)->InstanceEmpireFlag;
		eInstance.Render();
	}
}
...
void CPythonChat::AppendChat(int iType, const char * c_szChat, BYTE bEmpire, const char * szLanguage)
{
...
#if defined(ENABLE_LANGUAGE_SYSTEM)
	if (iType == CHAT_TYPE_SHOUT)
	{
		if (!bEmpire || !*szLanguage)
		{
			TraceError("CPythonChat::AppendChat - CANNOT_FIND_SENDER_LANGUAGE_AND_EMPIRE");
			return;
		}
		
		char szPathLanguage[CHAT_MAX_NUM + 1];
		sprintf(szPathLanguage, "d:/ymir work/ui/language_system/chat_language/%s.tga", szLanguage);
		if (CResourceManager::Instance().IsFileExist(szPathLanguage))
			pChatLine->InstanceLanguageFlag.SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer(szPathLanguage));
		
		char szPathEmpire[CHAT_MAX_NUM + 1];
		sprintf(szPathEmpire, "d:/ymir work/ui/language_system/chat_empire/empire_%d_256.dds", bEmpire);
		if (CResourceManager::Instance().IsFileExist(szPathEmpire))
			pChatLine->InstanceEmpireFlag.SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer(szPathEmpire));
	}
#endif
