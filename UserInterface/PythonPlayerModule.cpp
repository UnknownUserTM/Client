	...
	case 2:
		{
			...
			DWORD dwCount;
			if (!PyTuple_GetUnsignedLong(poArgs, 1, &dwCount))
				return Py_BuildException();

			if (0 == dwCount)
				return Py_BuildException();

			CPythonPlayer::Instance().SetItemCount(TItemPos (INVENTORY, iSlotIndex), dwCount);
			return Py_BuildNone();
		}
	case 3:
		{
			...
			DWORD dwCount;
			if (!PyTuple_GetUnsignedLong(poArgs, 2, &dwCount))
				return Py_BuildException();

			CPythonPlayer::Instance().SetItemCount(Cell, dwCount);
...
void initPlayer()
{
	...
	PyObject* poModule = Py_InitModule("fgGHGjjFHJghjfFG1545gGG", s_methods);
